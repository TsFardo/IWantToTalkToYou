...
<body>
    <div class='reg'>
    <h3><a class="a" href="index.php"><<<</a> Хочу с тобой пообщаться</h3>
    <h3>РЕГИСТРАЦИЯ</h3>
    <?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($username) && !empty($password1) && !empty($password2) && ($password1 == $password2)) {
            ...
            if ($kol == 0) {
                $secret = $password1;
                $data = date("Y-m-d");
                $ins = "INSERT INTO mismatch_user (username, password, email, join_date) VALUES (:username, :secret, :email, :data)";

                $inser = $pdo->prepare($ins);
                $inser->bindParam(':username', $username);
                $inser->bindParam(':secret', $secret);
