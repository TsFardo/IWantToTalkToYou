...
<body>
    <h3 class='h3'><a class='a' href="index.php"><<<</a> Хочу пообщаться - ПРОСМОТР ПРОФИЛЯ</h3>
    <?php
...
echo '</div>';
    if (!empty($row1['city']) || !empty($row1['state'])) {
        ...
        if (!empty($row1['picture'])) {
            echo '<tr><td class="label">Фотография:</td><td><img id="profileimg" src="' . MM_UPLOADPATH . $row1['picture'] . '" alt="Profile Picture" /></td></tr>';
        }
        echo '</table>';
        if (!isset($_GET['user_id']) || ($_SESSION['user_id'] == $_GET['user_id'])) {
